...
class RuTrackerParserEnhanced @Inject constructor(
     ...
     */
    suspend fun parseSearchResults(html: String): List<RuTrackerSearchResult> = withContext(Dispatchers.Default) {
        try {
     ...
     */
    suspend fun parseTorrentDetails(html: String): RuTrackerTorrentDetails = withContext(Dispatchers.Default) {
        try {
            val document = parseHtml(html)
     ...
     */
    suspend fun parseCategories(html: String): List<RuTrackerCategory> = withContext(Dispatchers.Default) {
        try {
            val document = parseHtml(html)
     ...
     */
    suspend fun parseMagnetLink(html: String): String? = withContext(Dispatchers.Default) {
        try {
            val document = parseHtml(html)
     ...
     */
    private suspend fun parseHtml(html: String): Document = withContext(Dispatchers.Default) {
        try {
            // Detect charset
    ...
    private fun parseSearchResultRow(row: Element): RuTrackerSearchResult? {
        try {
            // Extract topic ID
            val topicLink = row.select("a[href*=\"viewtopic.php?\"]").firstOrNull()
                ?: return null
            ...
            // Extract author
            val author = row.select("td:has(a[href*=\"profile.php\"])").firstOrNull()?.text()?.trim()
                ?: "Unknown"
            
            // Extract size
            val sizeText = row.select("td").find { td ->
            ...
            val seeders = row.select("td").find { td ->
                td.text().matches(Regex("\\d+"))
            }?.text()?.trim()?.toIntOrNull() ?: 0
            
            val leechers = row.select("td").find { td ->
                td.text().matches(Regex("\\d+"))
            }?.text()?.trim()?.toIntOrNull() ?: 0
            
            // Extract downloads
            val downloads = row.select("td").find { td ->
                td.text().matches(Regex("\\d+"))
            }?.text()?.trim()?.toIntOrNull() ?: 0
    ...
    private fun parseTitle(document: Document): String {
        return tryMultipleTextSelectors(
            document,
            listOf(
                "h1.maintitle",
                "h1",
    ...
    private fun parseSeeders(document: Document): Int {
        val seedersText = tryMultipleTextSelectors(
            document,
            listOf(
                ".seedmed",
                ".seeders",
    ...
    private fun parseLeechers(document: Document): Int {
        val leechersText = tryMultipleTextSelectors(
            document,
            listOf(
                ".leechmed",
                ".leechers",
    ...
    private fun parseDownloads(document: Document): Int {
        val downloadsText = tryMultipleTextSelectors(
            document,
            listOf(
                ".downloaded",
                ".downloads",
    ...
    private fun parseAddedDate(document: Document): Date? {
        val dateText = tryMultipleTextSelectors(
            document,
            listOf(
                ".posted",
                ".date",
    ...
    private fun parseCategoryLink(link: Element): RuTrackerCategory? {
        try {
            ...
            val categoryId = extractCategoryId(href)
                ?: return null
    ...
    private fun validateSearchResult(result: RuTrackerSearchResult): Boolean {
        return result.topicId.isNotEmpty() &&
    ...
    private fun validateTorrentDetails(details: RuTrackerTorrentDetails): Boolean {
        return details.title.isNotEmpty() &&
               details.size >= 0 &&
               details.seeders >= 0 &&
               details.leechers >= 0 &&
               details.downloads >= 0
