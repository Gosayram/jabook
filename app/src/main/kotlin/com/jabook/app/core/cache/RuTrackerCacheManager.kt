 ...
 */
@Singleton
class RuTrackerCacheManager @Inject constructor(
     ...
     */
    suspend fun remove(key: CacheKey): Result<Unit> = withContext(Dispatchers.IO) {
        mutex.withLock {
            val cacheKey = key.toString()
     ...
     */
    suspend fun clearNamespace(namespace: String): Result<Unit> = withContext(Dispatchers.IO) {
        mutex.withLock {
            try {
                // Remove from memory cache
     ...
     */
    suspend fun contains(key: CacheKey): Boolean = withContext(Dispatchers.IO) {
        mutex.withLock {
            val cacheKey = key.toString()
     ...
     */
    suspend fun getKeys(namespace: String): List<String> = withContext(Dispatchers.IO) {
        mutex.withLock {
    ...
    private suspend fun evictFromMemoryCache() {
        ...
        val entriesToEvict = memoryCache.entries
    ...
    private fun getDiskCacheFile(key: String): File {
        return File(cacheDir, "${key.replace(Regex("[^a-zA-Z0-9_-]"), "_")}.cache")
    }
    ...
    private fun estimateSize(obj: Any): Long {
        return try {
            Json.encodeToString(obj).toByteArray().size.toLong()
        } catch (e: Exception) {
    ...
    private suspend fun cleanupExpiredEntries() {
        mutex.withLock {
            ...
            // Clean memory cache
            val expiredMemoryKeys = memoryCache.keys.filter { key ->
     ...
     */
    suspend fun forceCleanup(): Result<Int> = withContext(Dispatchers.IO) {
        mutex.withLock {
            try {
