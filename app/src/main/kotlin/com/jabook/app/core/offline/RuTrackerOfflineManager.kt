 ...
 */
@Singleton
class RuTrackerOfflineManager @Inject constructor(
    ...
    suspend fun storeSearchResults(
        ...
        try {
            ...
            // Store in cache
            val persistentCacheKey = CacheKey(
     ...
     */
    suspend fun storeCategories(categories: List<RuTrackerCategory>): Result<Unit> = withContext(Dispatchers.IO) {
        try {
     ...
     */
    suspend fun getCategoriesOffline(): Result<List<RuTrackerCategory>> = withContext(Dispatchers.IO) {
        try {
            if (!isOfflineModeEnabled()) {
                return@withContext Result.failure(OfflineException("Offline mode is not enabled"))
            }
     ...
     */
    suspend fun getTorrentDetailsOffline(topicId: String): Result<RuTrackerTorrentDetails?> = withContext(Dispatchers.IO) {
        try {
            if (!isOfflineModeEnabled()) {
                return@withContext Result.failure(OfflineException("Offline mode is not enabled"))
            }
     ...
     */
    suspend fun loadOfflineData(): Result<Unit> = withContext(Dispatchers.IO) {
        try {
     ...
     */
    suspend fun clearOfflineData(): Result<Unit> = withContext(Dispatchers.IO) {
        try {
     ...
     * Update search index
     */
    private suspend fun updateSearchIndex(query: String, results: List<RuTrackerSearchResult>) {
        try {
        ...
        try {
            ...
            return allResults
                .filter { result ->
                    ...
                    val titleMatches = queryWords.count { queryWord ->
                        titleWords.any { titleWord -> 
                            titleWord.contains(queryWord) || queryWord.contains(titleWord)
                        }
                    }
                    
                    val authorMatches = queryWords.count { queryWord ->
     ...
     */
    private fun calculateRelevanceScore(result: RuTrackerSearchResult, query: String): Double {
        var score = 0.0
     ...
     * Calculate fuzzy match score
     */
    private fun calculateFuzzyMatchScore(result: RuTrackerSearchResult, query: String): Double {
        var score = 0.0
    ...
    private suspend fun updateOfflineDataStatus() {
        _offlineDataStatus.value = OfflineDataStatus(
     ...
     */
    private suspend fun recordOfflineSearchSuccess(query: String, resultCount: Int) {
        _offlineSearchAnalytics.update { current ->
            current.copy(
