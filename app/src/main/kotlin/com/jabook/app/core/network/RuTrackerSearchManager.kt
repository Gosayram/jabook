 ...
 */
@Singleton
class RuTrackerSearchManager @Inject constructor(
    ...
    suspend fun getSearchSuggestions(query: String, limit: Int = 10): List<String> {
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                val historySuggestions = searchHistory
                ...
                val popularSuggestions = popularSearches
                    .filter { it.query.contains(query, ignoreCase = true) }
                ...
                val suggestions = (historySuggestions + popularSuggestions)
                    .distinct()
                    .take(limit)
    ...
    suspend fun getSearchAnalytics(): SearchAnalyticsSummary {
        return withContext(Dispatchers.IO) {
            ...
            val averageResultsPerSearch = if (successfulSearches > 0) {
                searchAnalytics.values.sumOf { it.totalResults }.toDouble() / successfulSearches
            } else 0.0
            
            val successRate = if (totalSearches > 0) {
            ...
            SearchAnalyticsSummary(
                ...
                topQueries = searchAnalytics.entries
    ...
    private suspend fun performCategoryBrowsing(
        ...
        try {
            ...
            val filteredResults = categoryResults.filter { result ->
    ...
    private suspend fun getAudiobookCategoryId(): Int {
        return try {
            ...
            if (categories.isSuccess) {
                val audiobookCategory = categories.getOrNull()
     ...
     */
    private suspend fun browseCategory(categoryId: Int, page: Int): List<RuTrackerSearchResult> {
     ...
     * Record search attempt
     */
    private suspend fun recordSearchAttempt(query: String, categoryId: Int?) {
     ...
     */
    private suspend fun recordSearchFailure(query: String, categoryId: Int?) {
        val analyticsKey = "${query}_${categoryId}"
