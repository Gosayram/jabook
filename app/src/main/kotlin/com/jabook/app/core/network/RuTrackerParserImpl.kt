...
class RuTrackerParserImpl @Inject constructor() : RuTrackerParser {
    ...
    private fun extractTitle(html: String): String {
        return extractFromPattern(html, "<title>([^<]+)</title>", 1)
            ?.replace(" :: RuTracker.org", "")
            ?: "Unknown Title"
    }
    
    private fun extractAuthor(html: String): String {
        return extractFromPattern(html, "Автор[^:]*:([^<]+)", 1)
            ?: extractFromPattern(html, "Author[^:]*:([^<]+)", 1)
            ?: "Unknown Author"
    }
    
    private fun extractNarrator(html: String): String? {
        return extractFromPattern(html, "Исполнитель[^:]*:([^<]+)", 1)
            ?: extractFromPattern(html, "Narrator[^:]*:([^<]+)", 1)
    }
    
    private fun extractDescription(html: String): String {
        return extractFromPattern(html, "<div[^>]*class=\"[^\"]*post_body[^\"]*\"[^>]*>([^<]+)</div>", 1)
            ?.let { TextUtils.removeHtmlTags(it) }
            ?: ""
    }
    
    private fun extractCategory(html: String): String {
        return extractFromPattern(html, "Жанр[^:]*:([^<]+)", 1)
            ?: extractFromPattern(html, "Genre[^:]*:([^<]+)", 1)
            ?: "Unknown"
    }
    
    private fun extractSize(html: String): Long {
        val sizeText = extractFromPattern(html, "Размер[^:]*:([^<]+)", 1)
            ?: extractFromPattern(html, "Size[^:]*:([^<]+)", 1)
            ?: return 0
    ...
    private fun extractDuration(html: String): String? {
        return extractFromPattern(html, "Продолжительность[^:]*:([^<]+)", 1)
            ?: extractFromPattern(html, "Duration[^:]*:([^<]+)", 1)
    }
    
    private fun extractQuality(html: String): String {
        return extractFromPattern(html, "Качество[^:]*:([^<]+)", 1)
            ?: extractFromPattern(html, "Quality[^:]*:([^<]+)", 1)
            ?: "Unknown"
    }
    ...
    private fun extractAuthorFromRow(row: String): String {
        ...
        return extractFromPattern(row, "Автор[^:]*:([^<]+)", 1)
            ?: extractFromPattern(row, "Author[^:]*:([^<]+)", 1)
            ?: "Unknown"
    }
    ...
    private fun parseCategoryElement(element: String): Category? {
        return try {
            ...
            Category(
                id = id,
                name = TextUtils.removeHtmlTags(name)
            )
        } catch (e: Exception) {
            null
        }
