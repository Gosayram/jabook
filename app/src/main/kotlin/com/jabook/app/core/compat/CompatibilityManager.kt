...
/**
 * Compatibility manager for Android API 23-34 support
...
object CompatibilityManager {
     ...
     */
    fun isApiSupported(apiLevel: Int): Boolean {
    ...
    fun usesScopedStorage(): Boolean {
        return isApiSupported(Build.VERSION_CODES.R)
    }
     ...
     */
    fun hasBackgroundServiceRestrictions(): Boolean {
        return isApiSupported(Build.VERSION_CODES.O)
    }
    ...
    fun requiresNotificationPermission(): Boolean {
        return isApiSupported(Build.VERSION_CODES.TIRAMISU)
    }
    ...
    fun supportsMaterialYou(): Boolean {
        return isApiSupported(Build.VERSION_CODES.S)
    }
    
    /**
     * Check if per-app language preferences are available (API 33+)
     */
    fun supportsPerAppLanguage(): Boolean {
        return isApiSupported(Build.VERSION_CODES.TIRAMISU)
    }
}
...
object StorageCompat {
    ...
    fun getExternalStorageDirectory(context: Context): String {
        return if (CompatibilityManager.usesScopedStorage()) {
            // API 30+ uses scoped storage
...
object NotificationCompat {
    ...
    fun getNotificationImportance(priority: Int): Int {
        return if (CompatibilityManager.requiresNotificationChannels()) {
            when (priority) {
...
object ThemeCompat {
    ...
    fun supportsDynamicColors(): Boolean {
        return CompatibilityManager.supportsMaterialYou()
    }
    ...
    fun getSystemDarkMode(context: Context): Boolean {
        return if (supportsSystemDarkMode()) {
...
object AudioCompat {
    ...
    fun supportsAudioFocusRequest(): Boolean {
        return Build.VERSION.SDK_INT >= Build.VERSION_CODES.O
    }
    
    fun supportsMediaSession(): Boolean {
        return Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP
    }
    
    fun supportsAudioAttributes(): Boolean {
        return Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP
    }
    ...
    fun createAudioFocusRequest(
        ...
        return if (supportsAudioFocusRequest() && audioAttributes != null) {
