...
class FileManagerImpl
  ...
  constructor(
    ...
    override fun getAudiobooksDirectory(): File = File(context.filesDir, "audiobooks")

    override fun getTempDirectory(): File = File(context.cacheDir, "temp")

    override fun getCacheDirectory(): File = context.cacheDir

    override fun getLogsDirectory(): File = File(context.filesDir, "logs")

    override fun getAudiobookDirectory(audiobook: Audiobook): File =
      File(getAudiobooksDirectory(), "${audiobook.author}/${audiobook.title}")

    override fun getAudiobookAudioDirectory(audiobook: Audiobook): File = File(getAudiobookDirectory(audiobook), "audio")

    override fun getAudiobookMetadataFile(audiobook: Audiobook): File = File(getAudiobookDirectory(audiobook), "metadata.json")

    override fun getAudiobookCoverFile(audiobook: Audiobook): File = File(getAudiobookDirectory(audiobook), "cover.jpg")
    ...
    override suspend fun isAudiobookDownloaded(audiobook: Audiobook): Boolean = getAudiobookAudioDirectory(audiobook).exists()
    ...
    override suspend fun moveAudiobook(
      audiobook: Audiobook,
      newLocation: File,
    ): Boolean =
      try {
    ...
    override suspend fun importAudiobook(
      sourcePath: String,
      audiobook: Audiobook,
    ): Boolean =
      try {
    ...
    override suspend fun validateFileSystem(): FileSystemValidationResult =
      FileSystemValidationResult(isValid = true, issues = emptyList(), fixedIssues = emptyList())
  }
