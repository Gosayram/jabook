...
class RuTrackerParserImproved
  ...
  constructor(
    ...
    private fun processTorrentElement(
      doc: Document,
      selector: String,
    ): String? {
    ...
    private fun isApproved(statusText: String): Boolean = statusText.contains("проверено") || statusText.contains("approved")

    private fun isNotApproved(statusText: String): Boolean = statusText.contains("не проверено") || statusText.contains("not approved")

    private fun isNeedEdit(statusText: String): Boolean = statusText.contains("недооформлено") || statusText.contains("need edit")

    private fun isDubious(statusText: String): Boolean = statusText.contains("сомнительно") || statusText.contains("dubious")

    private fun isConsumed(statusText: String): Boolean = statusText.contains("поглощена") || statusText.contains("consumed")

    private fun isTemporary(statusText: String): Boolean = statusText.contains("временная") || statusText.contains("temporary")
    ...
    private fun extractTopicIdFromHref(href: String): String =
      href.substringAfter("t=").substringBefore("&").takeIf { it.isNotBlank() } ?: ""
    ...
    private fun extractSeeders(row: Element): Int = extractNumber(row, listOf(".seeders", ".seeds", "td:nth-child(6)", "td:nth-child(7)"))

    private fun extractLeechers(row: Element): Int = extractNumber(row, listOf(".leechers", ".leech", "td:nth-child(7)", "td:nth-child(8)"))
    ...
    private suspend fun parseAudiobookDetailsFromDocument(
      doc: Document,
      html: String,
    ): RuTrackerAudiobook? {
    ...
    private fun parseCategoryRow(row: Element): RuTrackerCategory? {
      try {
        val nameElement = row.selectFirst("a.gen")
        ...
        val href = nameElement.attr("href")
        val categoryId = href.substringAfter("f=").substringBefore("&").takeIf { it.isNotBlank() }
