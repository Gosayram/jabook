...
class RuTrackerApiServiceEnhanced
  ...
  constructor(
    ...
    class DomainUnavailableException(
      message: String,
    ) : Exception(message)
    ...
    class DetailsUnavailableException(
      message: String,
    ) : Exception(message)

    class CategoriesUnavailableException(
      message: String,
    ) : Exception(message)
    ...
    suspend fun searchAudiobooks(
      ...
      withContext(Dispatchers.IO) {
        ...
        val result =
          withBreaker(searchCircuitBreaker) {
    ...
    suspend fun getAudiobookDetails(
      ...
      withContext(Dispatchers.IO) {
        ...
        val result =
          withBreaker(detailsCircuitBreaker) {
    ...
    suspend fun getCategories(forceRefresh: Boolean = false): Result<List<RuTrackerCategory>> =
      withContext(Dispatchers.IO) {
        ...
        val result =
          withBreaker(categoriesCircuitBreaker) {
    ...
    private fun safeHandleError(
      ...
      try {
        val m1 =
          errorHandler.javaClass.methods.firstOrNull {
            it.name == "handleError" &&
              it.parameterCount == 3 &&
    ...
    private fun extractMagnetFromHtml(html: String): String? {
      ...
      Regex("""href\s*=\s*["'](magnet:[^"']+)["']""", RegexOption.IGNORE_CASE)
        .find(html)
        ?.groupValues
        ?.getOrNull(1)
        ?.let { return it }
    ...
    private fun CircuitBreaker.State.toCircuitState(): CircuitState =
      when (this) {
