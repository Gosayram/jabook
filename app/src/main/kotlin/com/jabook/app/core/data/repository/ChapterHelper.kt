...
class ChapterHelper
  ...
  constructor(
    ...
    fun getChaptersByAudiobookId(audiobookId: String): Flow<List<Chapter>> =
      chapterDao.getChaptersByAudiobookId(audiobookId).map { it.toChapterDomainList() }

    suspend fun getChapterById(id: String): Chapter? = chapterDao.getChapterById(id)?.toDomain()

    suspend fun getChapterByNumber(
      audiobookId: String,
      chapterNumber: Int,
    ): Chapter? = chapterDao.getChapterByNumber(audiobookId, chapterNumber)?.toDomain()
