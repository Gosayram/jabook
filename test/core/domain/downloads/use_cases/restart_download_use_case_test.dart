...
void main() {
  group('RestartDownloadUseCase', () {
    late TestDownloadsRepository testRepository;
    late RestartDownloadUseCase useCase;

    setUp(() {
      testRepository = TestDownloadsRepository();
      useCase = RestartDownloadUseCase(testRepository);
    });
    ...
    test('should restart download', () async {
      // Arrange
      ...
      final restartedDownload =
          downloads.firstWhere((d) => d.id == 'download-1');
      expect(restartedDownload.status, equals('downloading'));
      expect(restartedDownload.progress, equals(0.0));
      expect(restartedDownload.downloadedBytes, equals(0));
      expect(restartedDownload.isActive, isTrue);
    });
