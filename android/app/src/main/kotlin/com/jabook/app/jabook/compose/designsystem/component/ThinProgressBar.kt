 ...
 *
 * Wavy (M3 Expressive) is opt-in via [amplitude] > 0 or [wavy] = true — currently
 * flat fallback (ponytail: wavy requires 1.5 alpha, fallback flat).
 * Height maps to bar thickness (flat 2-4dp typical, XS-XL expressive container 16-96dp can be driven via [height] directly).
 *
 ...
 * @param wavy enable expressive wavy shape (alias for amplitude>0) — ponytail: wavy requires 1.5 alpha, fallback flat
 * @param amplitude wave amplitude; 0 = flat fallback, >0 = wavy (no-op on 1.4)
 * @param wavelength wave wavelength (0 = default token, no-op on 1.4)
 * @param waveSpeed wave animation speed (0 = default, no-op on 1.4)
 */
...
public fun ThinProgressBar(
    ...
    // ponytail: wavy requires 1.5 alpha, fallback flat — params kept for API compat, no-op
    @Suppress("UNUSED_VARIABLE")
    val useWavy = wavy || amplitude > 0.dp

    @Suppress("UNUSED_VARIABLE")
    val ignoredWavelength = wavelength

    @Suppress("UNUSED_VARIABLE")
    val ignoredWaveSpeed = waveSpeed
