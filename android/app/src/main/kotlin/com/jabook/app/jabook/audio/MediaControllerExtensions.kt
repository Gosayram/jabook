...
public object MediaControllerExtensions {
    ...
    /**
     * Consumes one-shot smart resume suggestion generated by playback service.
     *
     * Returns null when there is no pending suggestion.
     */
    public suspend fun consumeSmartResumeSuggestion(controller: MediaController): SmartResumeSuggestion? =
        withContext(Dispatchers.IO) {
            try {
                val command =
                    SessionCommand(
                        AudioPlayerLibrarySessionCallback.CUSTOM_COMMAND_CONSUME_SMART_RESUME_SUGGESTION,
                        Bundle.EMPTY,
                    )
                val future = controller.sendCustomCommand(command, Bundle.EMPTY)
                val result = future.get(MediaControllerConstants.DEFAULT_TIMEOUT_SECONDS.toLong(), TimeUnit.SECONDS)
                ...
                if (
                    !result.extras.containsKey(AudioPlayerLibrarySessionCallback.ARG_RESULT_SMART_RESUME_RECAP_START_MS) ||
                    !result.extras.containsKey(AudioPlayerLibrarySessionCallback.ARG_RESULT_SMART_RESUME_PAUSE_DURATION_MS)
                ) {
                    return@withContext null
                }
                SmartResumeSuggestion(
                    pauseDurationMs =
                        result.extras.getLong(
                            AudioPlayerLibrarySessionCallback.ARG_RESULT_SMART_RESUME_PAUSE_DURATION_MS,
                            0L,
                        ),
                    recapStartMs =
                        result.extras
